...
const toggleShare = asyncHandler(async (req, res, next) => {
  ...
  // If sharing for the first time or updating the doctor
  if (doctorId) {
    file.doctorId = String(doctorId);
    if (doctorName) file.doctorName = doctorName;
  } else if (!file.sharedWithDoctor && !file.doctorId) {
    // Fallback only if sharing for the first time and no doctorId provided
    try {
      const token = req.cookies.token || req.headers.authorization?.split(' ')[1];
      const doctorsResponse = await axios.get(`${process.env.AUTH_SERVICE_URL}/api/auth/doctors`, {
        headers: { Authorization: `Bearer ${token}` }
      });
      if (doctorsResponse.data && doctorsResponse.data.length > 0) {
        file.doctorId = String(doctorsResponse.data[0].id);
        file.doctorName = doctorsResponse.data[0].name;
      }
    } catch (err) {
      console.error('Failed to fetch doctor fallback:', err.message);
    }
  }

  // Toggle status if not explicitly setting a doctor (or if intended to toggle)
  // Usually, toggleShare is called to either turn it on or off
  file.sharedWithDoctor = !file.sharedWithDoctor;
  
  // If we just turned it ON and we have a doctor, ensure it stays ON
  if (doctorId && !file.sharedWithDoctor) {
      file.sharedWithDoctor = true; // Force ON if doctorId was provided (explicit share)
  }

  file.sharedAt = file.sharedWithDoctor ? new Date() : null;

  // If we just unshared the file, clear the doctor info so it doesn't "stick"
  ...
  if (file.sharedWithDoctor && file.doctorId) {
    try {
      await axios.post(`${process.env.NOTIFICATION_SERVICE_URL}/api/notifications/events`, {
        topic: 'file.shared',
        event: {
          fileId: file._id,
