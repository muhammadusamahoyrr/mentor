  ...
  try {
    ...
    if (!doctorInfo) {
      ...
      try {
        ...
        // Forward the caller's token — this lookup is no longer anonymous.
        const callerToken =
          req.cookies?.token || req.headers.authorization?.split(' ')[1];
        const { data } = await axios.get(`${authUrl}/api/auth/users/${doctorId}`, {
          timeout: 5000,
          headers: {
            Authorization: `Bearer ${callerToken}`,
            Cookie: `token=${callerToken}`,
          },
        });
    ...
    // Carry every field the notification-service caches. Omitting patientEmail /
    // doctorName wrote `undefined` into AppointmentCache, which the reminder job
    // then read back when composing emails.
    await Outbox.create([{
...
exports.deleteAppointment = asyncHandler(async (req, res, next) => {
  ...
  // Must be on the appointment — the old check only constrained patients, which
  // let any doctor delete any appointment in the system.
  if (!isAppointmentParticipant(appointment, req.user.id)) {
  ...
  // Delete and announce it atomically, so caches and notifications don't keep
  // referring to an appointment that no longer exists.
  const session = await mongoose.startSession();
  session.startTransaction();
  try {
