...
// True only when a real Redis is backing the store. When it is false we are on
// the in-process Map fallback, which is per-process and dies with the process —
// so callers must not treat "no session found" as proof of a logout.
redisCache.isBackedByRedis = () => redisAvailable && client.isOpen;
